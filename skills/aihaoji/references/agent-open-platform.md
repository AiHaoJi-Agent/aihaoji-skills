...
这个 skill 主要依赖开放平台的鉴权、笔记本管理、笔记列表、空白笔记创建、笔记详情、记录读取和笔记移动能力。
...
- [3.1 创建 Ai好记空白笔记](#31-创建-ai好记空白笔记)
...
- `note:create`：创建空白笔记并写入 Markdown 正文
- `folder:list`：读取笔记本树
- `folder:write`：创建、重命名、删除和移动笔记本
- `note:move`：移动单篇或批量移动笔记

写入接口返回 `401` / `403` 时，优先提示用户检查 API Key 和应用是否具备 `note:create`、`folder:write` 或 `note:move`。
...
- `content_markdown` 必填，最多 100000 个字符；服务端会转换为安全 HTML 保存，开放 HTML 会作为普通文本转义。
- `title` 可选，缺省时沿用现有空白笔记标题规则；`folder_id` 可选，缺省使用默认笔记本。
- 首版支持标题、粗体、斜体、删除线、列表、任务列表、表格、链接、自动 URL、代码块和引用等 Markdown；图片语法降级为链接，不支持附件、图片上传、`@` 引用、编辑、追加、删除或批量创建。
- 创建前必须展示标题、目标笔记本、正文摘要和字数，并等待用户明确确认；确认后才调用写接口。
- 创建成功后必须使用返回的 `note_id` 调用详情接口，传 `semantic_view=full` 回读正文验证。
