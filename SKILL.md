...
```http
GET /notes?page_no=1&page_size=10&folder_id=123&keyword=影视飓风
Authorization: $AIHAOJI_API_KEY
```
...
```http
GET /notes?page_no=1&page_size=10&keyword=影视飓风
Authorization: $AIHAOJI_API_KEY
```
...
当用户说“看一下影视飓风的文件夹”和“看一下影视飓风文件夹下的笔记”时，强制执行下面的固定流程：

1. 先调用 `GET /folders`
2. 在返回的笔记本树里定位名称为 `影视飓风` 的节点
3. 读取该节点的真实 `folder_id`
4. 再调用 `GET /notes?page_no=1&page_size=10&folder_id=<该 folder_id>`
   这一步里 `影视飓风` 只是 `folder_name`，不是 `keyword`
   这一跳必须显式视为 `keyword=None`
5. 把“当前笔记本信息 + 当前页笔记列表 + 如有更多页可继续翻页”一起展示给用户

禁止行为：

- 不允许把“影视飓风文件夹下的笔记”改写成 `keyword=影视飓风` 的关键词搜索
...
- 不允许一边说“这个笔记本有 9 篇”，一边只展示 `keyword=影视飓风` 命中的 5 篇还声称这是整本结果
...
如果 `/folders` 返回里已经存在用户点名的笔记本，例如 `影视飓风`，优先直接展开该节点所在分支，而不是改写成“当前可见结构视图”。
...
- 只有用户明确说“看总结 / 看大纲 / 看精华速览 / 看润色稿 / 看原文 / 看全文”这类视图型指令时，才允许直接走对应 `semantic_view`
- 也就是说：
  - “看这篇笔记” ≠ 默认看 `summary`
  - “查看详情” ≠ 默认先给详情概览
  - “打开详情” ≠ 默认展开正文
...
- 如果上下文里已经明确当前笔记，且用户意图是“查看详情 / 看详情 / 打开详情 / 看这篇笔记”，第一条回复只能给出 `A / B` 选择，不允许先给详情概览。
- `A`：导出 Markdown 到本地
  - 先调用 `GET /notes/{note_id}`；如果用户明确指定 `summary / outline / highlights / full / polish / original` 之一，再带上对应的 `semantic_view`
...
- `B`：直接在聊天里查看
  - 只有用户明确选择 `B` 后，才允许继续
  - 先返回详情概览，再按用户指定的部分继续展开
  - 如果用户要看的是 `full`、`polish`、`original` 这类长内容，必须使用 `semantic_view`，并从 `semantic_chunk_no=1` 开始
...
```text
我先把当前 AI好记 笔记本列表整理给你。你可以继续说“看影视飓风里的笔记”或“打开第 1 个笔记本”。
```
...
- 只有用户明确说“搜和影视飓风相关的笔记 / 搜关键词影视飓风”，才允许走全量关键词搜索链路
- 只有用户明确说“看影视飓风笔记本里的关于影视飓风的笔记 / 找一下影视飓风笔记本里和影视飓风相关的笔记”这类话术时，才允许走“笔记本内关键词搜索”链路

强制约束：

- “看影视飓风笔记本里的笔记”固定走 `/folders -> folder_id -> /notes?folder_id=...`
- 在这条链路里，`影视飓风` 仅表示 `folder_name`
...
```text
找一下影视飓风笔记本里的关于影视飓风的笔记
找一下影视飓风笔记本里和影视飓风相关的笔记
搜一下影视飓风笔记本里提到影视飓风的笔记
```

处理规则：

- 必须先调用 `GET /folders`
- 先把第一个“影视飓风”识别为 `folder_name`
- 定位真实 `folder_id`
- 再把“关于影视飓风 / 和影视飓风相关 / 提到影视飓风”识别为 `keyword=影视飓风`
...
- 不允许把 `keyword=影视飓风` 再解释成“严格相关”“标题明确提到”“讲影视飓风本身”等更窄的人工标准
